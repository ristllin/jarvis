...
class BrowserAgent:
    ...
    async def run(self, task, start_url=None, system_prompt=None,
                  tier="coding_level2", max_turns=30, temperature=0.2,
                  headless=True, viewport=None, continuation_context=None,
                  cookies=None, headers=None):
        log.info("browser_agent_start", task=task[:100], tier=tier, max_turns=max_turns)
        if self.blob:
            self.blob.store(event_type="browser_agent_start", content=f"Task: {task}",
                            metadata={"tier": tier, "max_turns": max_turns,
        ...
        try:
            ...
            msgs = self._init_msgs(task, start_url, system_prompt,
                                   continuation_context, await self._page_state())
            ...
            if p: ss.append(p)
            return {"success": False, "summary": f"Failed: {e}", "turns": len(acts),
                    "pages_visited": pgs, "screenshots": ss, "actions": acts, "extracted_data": data}
        finally:
            await self._cleanup()
    ...
    async def _loop(self, msgs, mt, tier, temp, ss, pgs, data, acts):
        for t in range(mt):
            try:
                r = await self.router.complete(messages=msgs, tier=tier, temperature=temp,
                                               max_tokens=4096, task_description=f"browser_agent:turn_{t}")
                c = r.content or ""
                if not c.strip():
                    msgs += [{"role": "assistant", "content": "(empty)"},
                ...
                if an == "navigate": pgs.append(act.get("url", ""))
                if an == "screenshot" and rt.startswith("Screenshot saved:"): ss.append(rt.split(": ",1)[1])
                ps = await self._page_state()
                msgs += [{"role": "assistant", "content": json.dumps(act)},
                         {"role": "user", "content": f"Result:\n{rt[:6000]}\n\n## Page\n{ps}"}]
                if len(msgs) > 40: msgs = msgs[:1] + msgs[-30:]
        ...
        return {"success": False, "summary": f"Max turns ({mt}).", "turns": mt,
                "pages_visited": pgs, "screenshots": ss, "actions": acts,
                "extracted_data": data, "continuation_context": msgs[-20:], "can_continue": True}
    ...
    def _done_result(self, act, t, ss, pgs, data, acts):
        ...
        if self.blob:
            self.blob.store(event_type="browser_agent_done", content=f"{s}\nTurns:{t+1}",
                            metadata={"turns": t+1, "pages_visited": pgs})
        return {"success": True, "summary": s, "turns": t+1, "pages_visited": pgs,
                "screenshots": ss, "actions": acts, "extracted_data": data}

    def _sys_prompt(self, custom=None):
        p = ["You are a browser automation agent, a subagent of JARVIS.",
             "You control a Chromium browser via Playwright.", ""]
        if custom: p.append(f"## Instructions\n{custom}\n")
        p.append(BROWSER_PRIMITIVES)
        return "\n".join(p)

    def _parse(self, c):
        for fn in [lambda: json.loads(c),
    ...
    async def _launch(self, headless, viewport, cookies, headers):
        ...
        kw = {"viewport": vp}
        if headers: kw["extra_http_headers"] = headers
        self._context = await self._browser.new_context(**kw)
        if cookies: await self._context.add_cookies(cookies)
    ...
    async def _page_state(self):
        try:
            ...
            txt = await self._page.evaluate(
                "document.body ? document.body.innerText.substring(0,1500) : '(empty)'")
            return f"URL: {url}\nTitle: {title}\nText:\n{txt[:1500]}"
        except Exception as e:
    ...
    async def _act_extract_text(self, a):
        ...
        if sel:
            ...
            txt = await self._page.evaluate(
                "document.body ? document.body.innerText : '(empty)'")
        return txt[:ml]
